    ...
    <section className={cx("space-y-4", className)}>
      {(title || description || actions) ? (
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            {title ? <h2 className="text-lg font-bold tracking-tight text-[var(--brand-text)]">{title}</h2> : null}
            {description ? <p className="mt-1 text-sm leading-6 text-[var(--brand-text-muted)]">{description}</p> : null}
          </div>
  ...
  return (
    <Surface className={cx("p-4", className)}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand-text-muted)]">
            {label}
          </p>
          <p className="mt-3 text-2xl font-extrabold tracking-tight text-[var(--brand-text)]">{value}</p>
        </div>
      ...
      {(helper || trend) ? (
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs font-semibold">
          ...
          {helper ? <span className="text-[var(--brand-text-muted)]">{helper}</span> : null}
        </div>
      ) : null}
    </Surface>
  );
...
export function EmptyState({ title = "Belum ada data", description, icon = "search", action }) {
  return (
    <div className="brand-empty-state brand-empty-state-with-motion">
      ...
      {description ? <p className="mt-2 max-w-md text-sm leading-7 text-[var(--brand-text-muted)]">{description}</p> : null}
  ...
  return (
    <Surface className={cx("p-4", className)}>
      {(title || description || action) ? (
        <div className="mb-4 flex items-start justify-between gap-4 border-b border-[var(--brand-border)] pb-3">
          <div>
            {title ? <h3 className="text-sm font-bold text-[var(--brand-text)]">{title}</h3> : null}
            {description ? <p className="mt-1 text-xs leading-5 text-[var(--brand-text-muted)]">{description}</p> : null}
          </div>
...
export function Drawer({ open, title, children, onClose, side = "right", className = "" }) {
  ...
  return (
    <div className="fixed inset-0 z-50" role="presentation">
      ...
      <aside
        className={cx(
          "brand-scrollbar absolute top-0 h-full w-[min(420px,calc(100vw-32px))] overflow-y-auto border-l border-[var(--brand-border)] bg-white shadow-2xl",
          sideClass,
          className
        )}
        role="dialog"
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-[var(--brand-border)] bg-white px-5 py-4">
          <h2 className="text-base font-bold text-[var(--brand-text)]">{title}</h2>
